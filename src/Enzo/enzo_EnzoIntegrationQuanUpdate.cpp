 ...
 const std::vector<std::vector<std::string>> &passive_lists) const noexcept
{
  auto clear_arr = [value,&dUcons_map](const std::string& key)
  {
    ...
    KView3D<enzo_float> kview3d("clear_dUcons_map_array",array.shape(0),array.shape(1),array.shape(2));

    auto h_kview3d = Kokkos::create_mirror_view(kview3d);
    for (int iz=0; iz<array.shape(0); iz++) {
      for (int iy=0; iy<array.shape(1); iy++) {
        for (int ix=0; ix<array.shape(2); ix++) {
          h_kview3d(iz,iy,ix) = array(iz,iy,ix);
        }
      }
    }
    Kokkos::deep_copy(kview3d,h_kview3d);

    Kokkos::parallel_for(
      Kokkos::MDRangePolicy< Kokkos::Rank<3> >( 
       {0,0,0},{array.shape(0),array.shape(1),array.shape(2)}),
       KOKKOS_LAMBDA (const int &iz, const int &iy, const int &ix){
          kview3d(iz,iy,ix) = value;
        });

    Kokkos::deep_copy(h_kview3d,kview3d);

    for (int iz=0; iz<array.shape(0); iz++) {
      for (int iy=0; iy<array.shape(1); iy++) {
        for (int ix=0; ix<array.shape(2); ix++) {
          array(iz,iy,ix) = h_kview3d(iz,iy,ix);
        }
      }
    }
